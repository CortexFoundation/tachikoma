...
/*!
 * \file src/relay/backend/contrib/tachikoma/codegen.cc
 * \brief Implementation of Tachikoma codegen APIs.
 */
...
class CodegenTachikoma : public MemoizedExprTranslator<std::vector<Output>>, public CodegenCBase {
 public:
  explicit CodegenTachikoma(const std::string& id) { this->ext_func_id_ = id; }

  std::vector<Output> VisitExprDefault_(const Object* op) final {
    LOG(FATAL) << "Tachikoma codegen doesn't support: " << op->GetTypeKey();
    return {};
  }
  ...
  std::vector<Output> VisitExpr_(const ConstantNode* cn) final {
    Output output;
    // Get const: static_cast<float*>(tachikoma_0_consts[0]->data)
 ...
 private:
  ...
  GenerateBodyOutput GenerateCompositeFunctionCall(const FunctionNode* callee,
    ...
    if (pattern_name == "tachikoma.conv2d_bias_relu") {
      const auto* conv_call =
          GetRootCall(callee->body.as<CallNode>(), 2, {"nn.conv2d", "add", "nn.relu"});
      return GenerateBody(conv_call, "tachikoma_fused_conv2d_bias_relu", GetArgumentNames(caller),
                          Conv2d(conv_call));
  ...
  /*! \brief The id of the external tachikoma ext_func. */
  ...
  int const_idx_{0};
  /*! \brief The arguments used by a wrapped function that calls Tachikoma kernels. */
  Array<Var> ext_func_args_;
  /*! \brief Statement of the function that will be compiled using Tachikoma kernels. */
  ...
  friend class TachikomaModuleCodegen;
};

/*!
 * \brief The Tachikoma codegen helper to generate wrapepr function calls of Tachikoma
 * libraries. The code is a CSourceModule that can be compiled separately and
 * linked together with a DSOModule.
 */
class TachikomaModuleCodegen : public CSourceModuleCodegenBase {
 public:
  // Create a corresponding Tachikoma function for the given relay Function.
  std::pair<std::string, Array<String>> GenTachikomaFunc(const Function& func) {
    ICHECK(func.defined()) << "Input error: expect a Relay function.";

    // Record the external symbol for runtime lookup.
    auto sid = GetExtSymbol(func);

    CodegenTachikoma builder(sid);
  ...
  runtime::Module CreateCSourceModule(const ObjectRef& ref) override {
    ...
    auto res = GenTachikomaFunc(Downcast<Function>(ref));
 ...
 */
runtime::Module TachikomaCompiler(const ObjectRef& ref) {
#ifdef USE_JSON_RUNTIME
  ICHECK(ref->IsInstance<FunctionNode>());
  ...
  return tachikoma.CreateCSourceModule(ref);
#endif
}
