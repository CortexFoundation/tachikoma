...
/*!
 * \file src/relay/backend/contrib/tachikoma/comp_op_matcher.h
 * \brief Implement matcher based function to parse complex composite nodes.
 */

#ifndef TVM_RELAY_BACKEND_CONTRIB_TACHIKOMA_COMP_OP_MATCHER_H_
#define TVM_RELAY_BACKEND_CONTRIB_TACHIKOMA_COMP_OP_MATCHER_H_
...
const tvm::relay::CallNode* ParseQnnConvComp(const tvm::relay::FunctionNode& comp_fn,
  ...
  ICHECK(res) << "Mismatch of Tachikoma partitioner and codegen logic";
...
const tvm::relay::CallNode* ParseComposite(const tvm::relay::FunctionNode& comp_fn,
  ...
  if (name == "tachikoma.qnn.conv2d")
    res = ParseQnnConvComp(comp_fn, ext_attrs, args);
  else if (name == "tachikoma.qnn.dense")
