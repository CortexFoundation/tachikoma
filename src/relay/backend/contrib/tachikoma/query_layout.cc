...
/*!
 * \file src/relay/backend/contrib/tachikoma/query_layout.cc
...
std::string md2fmt_tag_str(const tachikoma::memory::desc* md) {
...
tachikoma::memory::dims str2dims(const std::string& str_shape, bool dilates = false,
                            std::string interval = ",") {
  // Split strings
...
void check_shapes(const std::vector<std::string> shapes) {
  ...
  if (!checked) {
    LOG(FATAL) << "Invalid input args for query tachikoma optimal layout.";
  }
}

void check_layout(bool var, bool ref) {
  if (var != ref) {
    LOG(FATAL) << "Invalid input layout for query tachikoma optimal layout.";
  }
}
...
std::string get_optimal_layout_for_conv(std::string data_layout, std::string kernel_layout,
  ...
  tachikoma::engine eng(tachikoma::engine::kind::cpu, 0);
  tachikoma::stream s(eng);
  using tag = tachikoma::memory::format_tag;

  tachikoma::memory::dim groups = std::stoi(G);
  tachikoma::memory::dims weight_dims_ = str2dims(weight_shape);
  tachikoma::memory::dims weight_dims = weight_dims_;
  ...
  tachikoma::memory::dims out_dims = str2dims(out_shape);
  tachikoma::memory::dims padding_dims = str2dims(paddings);
  tachikoma::memory::dims padding_dims_l(padding_dims.begin(),
                                    padding_dims.begin() + padding_dims.size() / 2);
  tachikoma::memory::dims padding_dims_r(padding_dims.end() - padding_dims.size() / 2,
                                    padding_dims.end());
  tachikoma::memory::dims strides_dims = str2dims(strides);
  tachikoma::memory::dims dilates_dims = str2dims(dilates, true);

  tachikoma::memory::dims input_dims = out_dims;
  input_dims[1] = weight_dims[1];
  for (size_t i = 2; i < out_dims.size(); i++) {
    tachikoma::memory::dim K = weight_dims[i];
    tachikoma::memory::dim S = strides_dims[i - 2];
    tachikoma::memory::dim D = dilates_dims[i - 2];
    tachikoma::memory::dim PL = padding_dims_l[i - 2];
    tachikoma::memory::dim PR = padding_dims_r[i - 2];
    tachikoma::memory::dim DK = 1 + (K - 1) * (D + 1);
    input_dims[i] = out_dims[i] * S - PL - PR + DK - 1;
  }

  tachikoma::memory::dims conv_src_dims = input_dims;
  tachikoma::memory::dims conv_weights_dims = weight_dims;
  if (groups > 1) {
  ...
  tachikoma::memory::dims conv_dst_dims = out_dims;
  tachikoma::memory::dims conv_strides = strides_dims;
  tachikoma::memory::dims conv_dilates = dilates_dims;
  tachikoma::memory::dims conv_padding_l = padding_dims_l;
  tachikoma::memory::dims conv_padding_r = padding_dims_r;

  auto tachikoma_dtype = dtype_dl2tachikoma(tvm::runtime::String2DLDataType(dtype));
  auto conv_src_md = tachikoma::memory::desc({conv_src_dims}, tachikoma_dtype, tag::any);
  auto conv_weights_md = tachikoma::memory::desc({conv_weights_dims}, tachikoma_dtype, tag::any);
  auto conv_dst_md = tachikoma::memory::desc({conv_dst_dims}, tachikoma_dtype, tag::any);
  ...
  auto conv_prim_desc = tachikoma::convolution_forward::primitive_desc(conv_desc, eng);
...
std::string get_optimal_layout_for_conv_transpose(std::string data_layout,
  ...
  tachikoma::memory::dims padding_dims_r(padding_dims.end() - padding_dims.size() / 2,
                                    padding_dims.end());
  tachikoma::memory::dims output_padding_dims = str2dims(output_paddings);
  tachikoma::memory::dims strides_dims = str2dims(strides);
  tachikoma::memory::dims dilates_dims = str2dims(dilates, true);
  ...
  for (size_t i = 2; i < out_dims.size(); i++) {
    ...
    tachikoma::memory::dim PL = padding_dims_l[i - 2];
    tachikoma::memory::dim PR = padding_dims_r[i - 2];
    tachikoma::memory::dim OP = output_padding_dims[i - 2];
    tachikoma::memory::dim DK = 1 + (K - 1) * (D + 1);
    input_dims[i] = (out_dims[i] - DK + PL + PR - OP) / S + 1;
  }

  tachikoma::memory::dims deconv_src_dims = input_dims;
  tachikoma::memory::dims deconv_weights_dims = weight_dims;
  if (groups > 1) {
  ...
  tachikoma::memory::dims deconv_dst_dims = out_dims;
  tachikoma::memory::dims deconv_strides = strides_dims;
  tachikoma::memory::dims deconv_dilates = dilates_dims;
  tachikoma::memory::dims deconv_padding_l = padding_dims_l;
  tachikoma::memory::dims deconv_padding_r = padding_dims_r;

  auto tachikoma_dtype = dtype_dl2tachikoma(tvm::runtime::String2DLDataType(dtype));
  auto deconv_src_md = tachikoma::memory::desc({deconv_src_dims}, tachikoma_dtype, tag::any);
  auto deconv_weights_md = tachikoma::memory::desc({deconv_weights_dims}, tachikoma_dtype, tag::any);
  auto deconv_dst_md = tachikoma::memory::desc({deconv_dst_dims}, tachikoma_dtype, tag::any);

  auto deconv_desc = tachikoma::deconvolution_forward::desc(
      tachikoma::prop_kind::forward_inference, tachikoma::algorithm::deconvolution_direct, deconv_src_md,
      deconv_weights_md, deconv_dst_md, deconv_strides, deconv_dilates, deconv_padding_l,
      deconv_padding_r);

  auto deconv_prim_desc = tachikoma::deconvolution_forward::primitive_desc(deconv_desc, eng);
