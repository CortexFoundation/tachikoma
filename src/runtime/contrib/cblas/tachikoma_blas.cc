...
inline char TachikomaBooleanToTransposeChar(bool trans) { return trans ? 'T' : 'N'; }

struct TachikomaSgemmOp {
...
TVM_REGISTER_GLOBAL("tvm.contrib.tachikoma.matmul").set_body([](TVMArgs args, TVMRetValue* ret) {
  DLTensor* A = args[0];
  ICHECK(TypeMatch(A->dtype, kDLFloat, 32));
  CallGemm(args, ret, TachikomaSgemmOp());
});
}  // namespace contrib
}  // namespace tvm
