...
class TachikomaJSONRuntime : public JSONRuntimeBase {
 public:
  ...
  void Run(const TVMArgs& args) const {
    ...
    for (const auto& act : net_) {
      ...
      if (prim.get_kind() == tachikoma::primitive::kind::reorder) {
 ...
 private:
  ...
  tachikoma::primitive_attr ParseAttrs(const size_t& nid, TensorRequisite* bias_tr) {
    ...
    *bias_tr = GetInputByName(nid, "bias_idx");
    ...
    if (std::regex_match(op_name, clip_pat)) {
      ...
      ops.append_eltwise(1.f, tachikoma::algorithm::eltwise_clip, a_min, a_max);
    }
    ...
    if (std::regex_match(op_name, swish_pat)) {
      ops.append_eltwise(1.f, tachikoma::algorithm::eltwise_swish, 1.f, 1.f);
    }
    ...
    if (std::regex_match(op_name, mish_pat)) {
      ops.append_eltwise(1.f, tachikoma::algorithm::eltwise_mish, 1.f, 0.f);
    }
    if (ops.len() != 0) {
      attr.set_post_ops(ops);
    }
  ...
  void Convolution(const size_t& nid) {
    ...
    std::cerr << "Trace:\n-----" << std::endl;
    std::cerr << (int)(src_tr.get_data_handle()) << std::endl;
  ...
  void BatchMatMul(const size_t& nid) {
    ...
    // Matmul description.
    auto bmm_desc = tachikoma::matmul::desc(src_tr.LayoutAny().desc(), wgh_tr.LayoutAny().desc(),
                                       bias_tr.LayoutAny().desc(), dst_tr.LayoutAny().desc());

    // Enable elementwise post-ops.
    auto bmm_prim_desc = tachikoma::matmul::primitive_desc(bmm_desc, attr, engine_);
    ...
    Submit(tachikoma::matmul(bmm_prim_desc), {{DNNL_ARG_SRC, src_tr},
                                         {DNNL_ARG_WEIGHTS, wgh_tr},
                                         {DNNL_ARG_BIAS, bias_tr},
                                         {DNNL_ARG_SCRATCHPAD, scratchpad_tr},
                                         {DNNL_ARG_DST, dst_tr}});
  ...
  void Pooling(const size_t& nid, tachikoma::algorithm algo) {
    ...
    if (!is_global && algo == tachikoma::algorithm::pooling_avg) {
