...
from tvm.relay.op.contrib.tachikoma import partition_for_tachikoma
import tvm.testing
import numpy as np
...
def run_and_verify(mod, input, params, target, run_module):
    ...
    for mode in ["graph", "vm"]:
        for use_tachikoma, alter_layout in [(False, False), (True, False), (True, True)]:
            result_key = mode + ("_tachikoma" if use_tachikoma else "") + ("_layout" if alter_layout else "")
            if use_tachikoma:
                processed_mod = partition_for_tachikoma(mod, params, alter_layout)
